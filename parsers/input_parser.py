...
def parse_base_city() -> City:
    ...
    return City(city_name, airport_code, -1, -1)

def parse_city() -> City:
    city_info = readline()
    city_name, airport_code, min_nights, max_nights = city_info.split(' ')
    min_nights = int(min_nights)
    max_nights = int(max_nights)
    return City(city_name, airport_code, min_nights, max_nights)
...
def parse_flight(flight_list : list[Flight]) -> Flight:
    ...
    cost = int(cost)
    flight = Flight(day, departure_city, arrival_city, departure_time, arrival_time, cost)
    ...
    if len(duplicates) > 0:
        ...
        flight = Flight(day, departure_city, arrival_city, departure_time, arrival_time, min_cost)
